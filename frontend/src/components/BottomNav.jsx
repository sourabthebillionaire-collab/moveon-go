    ...
    <nav className="bottom-nav">
      {TABS.map(tab => (
        <NavLink
          ...
          {({ isActive }) => (
            <>
              <div className={`bnav-icon-wrap ${isActive ? 'bnav-icon-wrap--active' : ''}`}>
...
function HomeIcon({ active }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>
    <polyline points="9 22 9 12 15 12 15 22" stroke="currentColor" fill="none"/>
  </svg>;
}
function BusIcon({ active }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round">
